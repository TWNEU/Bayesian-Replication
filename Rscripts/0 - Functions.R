eb_prepare <- function(data, scale_alphas, overlapping) {
  if (overlapping) {
    data <- data %>%
      ...
      mutate(obs = n()) %>%
      ungroup() %>%
      # If overlapping obs, select only groups with max num of obs
      filter(obs == max(obs)) %>%
      select(-obs)
  }
  # Adjust for Beta
  data <- data %>% 
    ...
    mutate(
      ...
      # Adjust returns for market exposure
      ret_neu = (ret - mkt_vw_exc * beta)*100,
      # Adjust for scaling, target annualized volatility 10%
      scaling_fct = sqrt(10^2/12) / sd(ret_neu),
      # Scaled version of adjusted returns
...
# Transform raw correlation matrix into block-structured correlation matrix
...
emp_bayes <- function(data, cluster_labels, min_obs = 5 * 12, fix_alpha = F, bs_cov = F, cor_type = "sample", shrinkage = 0, layers = 3, bs_samples = 10000, seed, priors = NULL, sigma = NULL, plot = T) { # cor_type %in% c("raw", "block_2", "block_clusters")
  ...
  # Covariance Estimation (Bootstrap or Raw)
  if (is.null(sigma)) {
    ...
    if (bs_cov) {
      ...
      # Calculate standard deviations of each characteristic
      alpha_sd <- sqrt(diag(bs_full_cov)) 
      # Transform covariance matrix bs_full_cov into correlation matrix
