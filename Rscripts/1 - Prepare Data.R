...
# Labels 
char_info <- readxl::read_xlsx(paste0(data_path, "/Factor_Details.xlsx"),
...
country_classification <- readxl::read_xlsx(paste0(data_path, "/Country_Classification.xlsx"), 
...
hml <- hml %>%
  filter(
    ...
    eom >= settings$start_date & eom <= settings$end_date,
    !is.na(ret) & n_stocks_min >= settings$n_stocks_min, 
    !(excntry %in% settings$country_excl)
  ) %>%
